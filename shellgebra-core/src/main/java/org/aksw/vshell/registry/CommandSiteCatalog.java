...
public interface CommandSiteCatalog {
    /**
     ...
     */
    Optional<Multimap<ExecSite, CommandBinding>> get(String virtualCommandName);
    ...
    default Optional<Set<CommandBinding>> get(String virtualCommandName, ExecSite execSite) {
        Multimap<ExecSite, CommandBinding> mm = get(virtualCommandName).orElse(null);
        Set<CommandBinding> result = null;
        if (mm != null) {
            result = (Set<CommandBinding>) mm.asMap().get(execSite);
        }
        return Optional.ofNullable(result);
    }

    /** Convenience method to get only a command's set of exec sites. */
    default Optional<Set<ExecSite>> getExecSites(String virtualCommandName) {
        return get(virtualCommandName).map(Multimap::keySet);
    }
}
